...
class FeastOpenLineageEmitter:
    ...
    def emit_registry_lineage(
        ...
        # Emit events for data sources
        try:
            data_sources = registry.list_data_sources(
                project=project, allow_cache=allow_cache
            )
            for ds in data_sources:
                result = self.emit_data_source_lineage(ds, project)
                results.append(result)
        except Exception as e:
            logger.error(f"Error emitting data source lineage: {e}")

        # Emit events for entities
        try:
            entities = registry.list_entities(project=project, allow_cache=allow_cache)
            for entity in entities:
                result = self.emit_entity_lineage(entity, project)
                results.append(result)
        except Exception as e:
            logger.error(f"Error emitting entity lineage: {e}")
    ...
    def emit_data_source_lineage(
        self,
        data_source: "DataSource",
        project: str,
    ) -> bool:
        """
        Emit lineage for a data source.
        ...
        from feast.openlineage.mappers import data_source_to_dataset

        try:
            namespace = self._get_namespace(project)
            dataset = data_source_to_dataset(
                data_source,
                namespace=namespace,
                as_input=True,
            )
    ...
    def emit_entity_lineage(
        self,
        entity: "Entity",
        project: str,
    ) -> bool:
        """
        Emit lineage for an entity.
        ...
        from feast.openlineage.mappers import entity_to_dataset

        try:
            namespace = self._get_namespace(project)
            dataset = entity_to_dataset(
                entity,
                namespace=namespace,
            )
            ...
            logger.error(f"Error emitting entity lineage for {entity.name}: {e}")
            return False
    ...
    def emit_historical_retrieval(
        ...
        """
        Emit lineage for a historical feature retrieval.

        Args:
            feature_refs: List of feature references
            feature_service: Optional feature service used
        ...
        Returns:
            True if successful, False otherwise
        """
        if not self.is_enabled or not self._config.emit_on_retrieval:
            return False

        from feast.openlineage.facets import FeastRetrievalFacet

        try:
            run_id = str(uuid.uuid4())
            namespace = self._get_namespace(project)

            # Build inputs (feature views)
            inputs = []
            feature_views_used = set()

            for ref in feature_refs:
                if ":" in ref:
                    fv_name = ref.split(":")[0]
                    feature_views_used.add(fv_name)

            for fv_name in feature_views_used:
                inputs.append(
                    InputDataset(
                        namespace=namespace,
                        name=fv_name,
            ...
            # Build output (the retrieved DataFrame)
            outputs = [
                OutputDataset(
                    namespace=namespace,
                    name=f"historical_features_{run_id[:8]}",
                )
            ]

            # Determine entity count
            entity_count = None
            if hasattr(entity_df, "__len__"):
                entity_count = len(entity_df)
            elif hasattr(entity_df, "count"):
                try:
                    entity_count = entity_df.count()
                except Exception:
                    pass

            run_facets = {
                "feast_retrieval": FeastRetrievalFacet(
                    retrieval_type="historical",
                    feature_service=feature_service.name if feature_service else None,
                    feature_views=list(feature_views_used),
                    features=feature_refs,
                    entity_count=entity_count,
                )
            }

            # Emit as a complete run (single event for retrieval)
            return self._client.emit_run_event(
                job_name="get_historical_features",
                run_id=run_id,
                event_type=RunState.COMPLETE,
                inputs=inputs,
                outputs=outputs,
    ...
    def emit_online_retrieval(
        ...
        """
        Emit lineage for an online feature retrieval.

        Args:
            feature_refs: List of feature references
            feature_service: Optional feature service used
            entity_rows: Entity rows for the request
            project: Project name
        ...
        try:
            ...
            for fv_name in feature_views_used:
                inputs.append(
                    InputDataset(
                        namespace=f"{namespace}/online_store",
                        name=fv_name,
                    )
                )
            ...
            outputs = [
                OutputDataset(
                    namespace=namespace,
                    name=f"online_features_{run_id[:8]}",
                )
            ]
            ...
            run_facets = {
                "feast_retrieval": FeastRetrievalFacet(
                    ...
                    feature_views=list(feature_views_used),
                    features=feature_refs,
                    entity_count=len(entity_rows) if entity_rows else 0,
                )
            }

            return self._client.emit_run_event(
                job_name="get_online_features",
                run_id=run_id,
                event_type=RunState.COMPLETE,
                inputs=inputs,
                outputs=outputs,
            ...
            logger.error(f"Error emitting online retrieval lineage: {e}")
            return False
