...
class TestGetK8sApiClient:
    ...
    @patch("feast.infra.compute_engines.spark.byos.auth.config")
    @patch("feast.infra.compute_engines.spark.byos.auth.client")
    def test_kubeconfig_auth(self, mock_client, mock_config):
        mock_api_client = MagicMock()
        mock_client.ApiClient.return_value = mock_api_client
    ...
    def test_nonexistent_kubeconfig_raises(self):
        with pytest.raises(FeastSparkClusterError, match="kubeconfig file not found"):
            get_k8s_api_client(kubeconfig_path="/nonexistent/kubeconfig")

    @patch("feast.infra.compute_engines.spark.byos.auth.config")
    def test_in_cluster_auth_failure_wraps_error(self, mock_config):
        mock_config.load_incluster_config.side_effect = Exception(
            "Service account not found"
        )

        with pytest.raises(
            FeastSparkClusterError, match="Failed to authenticate with Kubernetes"
        ):
            get_k8s_api_client(kubeconfig_path=None)

    @patch("feast.infra.compute_engines.spark.byos.auth.config")
    def test_kubeconfig_load_failure_wraps_error(self, mock_config):
        mock_config.load_kube_config.side_effect = Exception("Invalid kubeconfig")
    ...
    @patch("feast.infra.compute_engines.spark.byos.auth.config")
    @patch("feast.infra.compute_engines.spark.byos.auth.client")
    def test_kubeconfig_tilde_expansion(self, mock_client, mock_config):
        mock_api_client = MagicMock()
        mock_client.ApiClient.return_value = mock_api_client

        with tempfile.NamedTemporaryFile(
            dir=os.path.expanduser("~"), suffix=".kubeconfig", delete=False
        ) as f:
            f.write(b"apiVersion: v1\n")
            full_path = f.name
