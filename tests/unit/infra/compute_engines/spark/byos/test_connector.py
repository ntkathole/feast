...
class TestBYOSConnectorValidateConnectivity:
    """Test validate_connectivity with mocked K8s client."""

    @patch("kubernetes.client.CoreV1Api")
    @patch("feast.infra.compute_engines.spark.byos.auth.get_k8s_api_client")
    def test_successful_connectivity(self, mock_get_client, mock_core_v1):
    ...
    @patch("feast.infra.compute_engines.spark.byos.auth.get_k8s_api_client")
    def test_auth_failure_raises_cluster_error(self, mock_get_client):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        with pytest.raises(FeastSparkClusterError, match="Failed to authenticate"):
            connector.validate_connectivity(config)

    @patch("kubernetes.client.CoreV1Api")
    @patch("feast.infra.compute_engines.spark.byos.auth.get_k8s_api_client")
    def test_unreachable_cluster_raises(self, mock_get_client, mock_core_v1):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
    ...
    @patch("kubernetes.client.CoreV1Api")
    @patch("feast.infra.compute_engines.spark.byos.auth.get_k8s_api_client")
    def test_uses_kubeconfig_path(self, mock_get_client, mock_core_v1):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
    ...
    @patch("kubernetes.client.CoreV1Api")
    @patch("feast.infra.compute_engines.spark.byos.auth.get_k8s_api_client")
    def test_custom_namespace(self, mock_get_client, mock_core_v1):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
...
class TestBYOSConnectorCreateSparkSession:
    """Test create_spark_session builds correct SparkConf."""

    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_basic_remote_session(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        spark_conf = call_args.kwargs.get("conf") or call_args[1].get("conf") or call_args[0][0]
        conf_dict = dict(spark_conf.getAll())
        assert conf_dict["spark.master"] == "k8s://https://kubernetes.default.svc"
        assert conf_dict["spark.kubernetes.namespace"] == "default"

    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_session_with_images(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        spark_conf = call_args.kwargs.get("conf") or call_args[1].get("conf") or call_args[0][0]
        conf_dict = dict(spark_conf.getAll())
        assert conf_dict["spark.kubernetes.container.image"] == "feast/spark:base"
        assert conf_dict["spark.kubernetes.driver.container.image"] == "feast/spark:driver"
        assert conf_dict["spark.kubernetes.executor.container.image"] == "feast/spark:executor"

    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_session_with_service_account(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        spark_conf = call_args.kwargs.get("conf") or call_args[1].get("conf") or call_args[0][0]
        conf_dict = dict(spark_conf.getAll())
        assert conf_dict["spark.kubernetes.authenticate.driver.serviceAccountName"] == "spark-sa"
        assert conf_dict["spark.kubernetes.authenticate.executor.serviceAccountName"] == "spark-sa"

    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_session_with_secrets(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        spark_conf = call_args.kwargs.get("conf") or call_args[1].get("conf") or call_args[0][0]
        conf_dict = dict(spark_conf.getAll())
        assert conf_dict["spark.kubernetes.driver.secrets.s3-creds"] == "/mnt/s3"
        assert conf_dict["spark.kubernetes.executor.secrets.s3-creds"] == "/mnt/s3"

    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_session_with_resource_config(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
    ...
    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_user_spark_conf_overrides(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        config = _make_remote_config(
            spark_conf={"spark.executor.instances": "8", "spark.custom.key": "value"},
        )
        connector = BYOSConnector()
        connector.create_spark_session(config)
    ...
    @patch("feast.infra.compute_engines.spark.byos.connector.SparkSession")
    def test_image_pull_secrets(self, mock_spark_session):
        from feast.infra.compute_engines.spark.byos.connector import BYOSConnector
        ...
        config = _make_remote_config(image_pull_secrets=["regcred", "ghcr-secret"])
        connector = BYOSConnector()
        connector.create_spark_session(config)
        ...
        spark_conf = call_args.kwargs.get("conf") or call_args[1].get("conf") or call_args[0][0]
        conf_dict = dict(spark_conf.getAll())
        assert conf_dict["spark.kubernetes.container.image.pullSecrets"] == "regcred,ghcr-secret"
