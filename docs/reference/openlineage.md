...
- **Feature retrieval** - Events when features are retrieved (online and historical)
...
openlineage:
  ...
  emit_on_retrieval: false  # Disabled by default to reduce event volume
```
...
# Feature retrieval emits events automatically (if emit_on_retrieval is enabled)
...
| `emit_on_retrieval` | `false` | Emit events on feature retrieval |
...
- `feature_service`: Name of feature service used
- `features`: List of features retrieved
...
| Feature Retrieval | RunEvent (COMPLETE) |
