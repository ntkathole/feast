...
class FeatureStore:
    ...
    def _emit_openlineage_historical_retrieval(
        self,
        feature_refs: List[str],
        feature_service: Optional[FeatureService],
        entity_df: Any,
    ):
        """Emit OpenLineage event for historical feature retrieval."""
        if self._openlineage_emitter is None:
            return
        try:
            self._openlineage_emitter.emit_historical_retrieval(
                feature_refs, feature_service, entity_df, self.project
            )
        except Exception as e:
            warnings.warn(f"Failed to emit OpenLineage historical retrieval event: {e}")
    ...
    def get_online_features(
        ...
        # Emit OpenLineage event for online retrieval
        self._emit_openlineage_online_retrieval(
            features,
            entity_rows,
        )
    ...
    def _emit_openlineage_online_retrieval(
        self,
        features: Union[List[str], FeatureService],
        entity_rows: Any,
    ):
        """Emit OpenLineage event for online feature retrieval."""
        if self._openlineage_emitter is None:
            return
        try:
            ...
            if isinstance(features, FeatureService):
                feature_refs = [
                    f"{proj.name}:{f.name}"
                    for proj in features.feature_view_projections
            ...
            # Convert entity_rows to list if needed
            if isinstance(entity_rows, list):
                rows = entity_rows
            elif hasattr(entity_rows, "items"):
                # It's a mapping
                rows = [dict(entity_rows)]
            else:
                rows = []

            self._openlineage_emitter.emit_online_retrieval(
                feature_refs, feature_service, rows, self.project
            )
        except Exception as e:
            warnings.warn(f"Failed to emit OpenLineage online retrieval event: {e}")
